...
Value * IDISA_ARM_Builder::hsimd_pairwisesum(unsigned fw, llvm::Value * Val_a, llvm::Value * Val_b){
  
  if (mBitBlockWidth == 128 && fw == 8) {
    Function * shuf8Func = Intrinsic::getDeclaration(getModule(), Intrinsic::aarch64_neon_addp, FixedVectorType::get(getInt8Ty(), 16));
    return fwCast(8, CreateCall(shuf8Func->getFunctionType(), shuf8Func, {fwCast(8, Val_a), fwCast(8, Val_b)}));
  }
  return IDISA_Builder::hsimd_pairwisesum(fw, Val_a, Val_b);
}
